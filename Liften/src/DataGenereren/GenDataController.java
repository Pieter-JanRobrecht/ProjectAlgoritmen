...
public class GenDataController {
    ...
    @FXML
    protected ComboBox<String> spreidingCombo;
    ...
    public void generateJson() {
        ...
        for (int i = 0; i < amountOfLifts; i++) {
            ...
            if (random.nextDouble() > brokenElevators / 100) {
                lft.setOpeningTime(openingTime * (1 - offsetPercentage + 2 * offsetPercentage * random.nextDouble()));
                lft.setClosingTime(closingTime * (1 - offsetPercentage + 2 * offsetPercentage * random.nextDouble()));
            } else {
                lft.setOpeningTime((1 - offsetPercentage + 2 * offsetPercentage * random.nextDouble()) * openingTime * 5);
                lft.setClosingTime((1 - offsetPercentage + 2 * offsetPercentage * random.nextDouble()) * closingTime * 5);
            }


            for (int j = 0; j < amountOfLevels; j++) {
                if ((random.nextDouble()) > chanceOfStopOnFloor) {
                    Range rng = new Range();
                    rng.setId(j);
                    levelsReached[j] = true;
                    range.add(rng);
        ...
        for (int i = 0; i < amountOfUsers; i++) {
            ...
            switch (spreiding) {
                case "'s Ochtends":
                    if (i < amountOfUsers / 3)
                        temp = arrivalTime + random.nextInt(tempoOfArrival / 2) * (1 + offsetPercentage * random.nextDouble());
                    else
                        temp = arrivalTime + random.nextInt(tempoOfArrival) * (1 + offsetPercentage * random.nextDouble());
                    usr.setArrivalTime(temp);
                    arrivalTime = temp;
                    break;
                case "'s Middags":
                    if (i > amountOfUsers / 3 && i < amountOfUsers * 0.6666)
                        temp = arrivalTime + random.nextInt(tempoOfArrival / 2) * (1 + offsetPercentage * random.nextDouble());
                    else
                        temp = arrivalTime + random.nextInt(tempoOfArrival) * (1 + offsetPercentage * random.nextDouble());
                    usr.setArrivalTime(temp);
                ...
                case "'s Avonds":
                    ...
                    arrivalTime = temp;
                    break;
                case "Gelijktijdig verdeeld over de dag":
                    temp = arrivalTime + random.nextInt(tempoOfArrival);
                    usr.setArrivalTime(temp);
                    arrivalTime = temp;
                    break;
                default:
            ...
            if (random.nextDouble() > oldAgePercentage / 100) {
                usr.setUnboardingTime(tempUnboardingTime);
                usr.setBoardingTime(tempBoardingTime);
            } else {
                usr.setBoardingTime(2 * tempBoardingTime);
                usr.setUnboardingTime(2 * tempUnboardingTime);
            }
            ...
            while (temp2 == usr.getSourceId()) {
                temp2 = random.nextInt(amountOfLevels);
            }
        ...
        controller.getApplication().reset((Stage) statusLabel.getScene().getWindow());
    }
