...
public class Simulation {
	...
	public Simulation(ManagementSystem ms) {
		ec = new ElevatorController(ms); //is dit copy by reference or copy by value? (call..)
		mainTicker = 0;
		succes = 0;
		failure = 0;
		queue = new ArrayList<>();
	...
	/** Configurations:
	 * Set currents users = 0
	 * Set current level = startlevel
	 * current direction is 0 --> +1 = UP | -1 = DOWN
	 * 
	 * Start = no elevators moving, assign CLOSEST elevator with NO direction to first user
	 * if no elevators with NO direction, check elevators with direction towards user.
	 * 
	 * 
	...
	public void addValidUsers(int time) {
		...
		for(int i = 0; i < ec.getUsers().size(); i++) {
			System.out.println("DEBUG - " + ec.getUsers().get(i).getArrivalTime() + " < " + time + " => " + (ec.getUsers().get(i).getArrivalTime() < mainTicker));
	...
	public List<User> getValidUsers(int time) {
		...
		for(int i = 0; i < ec.getUsers().size(); i++) {
			System.out.println("DEBUG - " + ec.getUsers().get(i).getArrivalTime() + " < " + time + " => " + (ec.getUsers().get(i).getArrivalTime() < mainTicker) + " (to returnList)");
			if(ec.getUsers().get(i).getArrivalTime() < time) {
	...
	public Lift assignElevator(User u) {
		...
		for(Lift l : ec.getLifts()) {
			if(l.getDirection() == 0) {
				if(distance > Math.abs(u.getSourceId()-l.getCurrentLevel()) && l.getCurrentUsers() < l.getCapacity()) {
					returnLift = l;
		...
		//check if we can assign a lift which is in use  || WORK IN PROGRESSSSSSSS
		for(Lift l : ec.getLifts()) {
